                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.users.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="role" name="role" class="border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm w-full" required>
                                <option value="leader" {{ old('role') === 'leader' ? 'selected' : '' }}>Leader</option>
                                <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Admin</option>
                            </select>
                            <x-input-error :messages="$errors->get('role')" class="mt-2" />
                            <p class="text-sm text-gray-600 mt-1">Admins can manage leaders but not other admins or superadmins</p>
                        </div>
                        @endif
