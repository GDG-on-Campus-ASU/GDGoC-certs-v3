    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('dashboard.templates.certificates.update', $certificateTemplate) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select id="type" name="type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="svg" {{ old('type', $certificateTemplate->type) === 'svg' ? 'selected' : '' }}>SVG</option>
                                <option value="blade" {{ old('type', $certificateTemplate->type) === 'blade' ? 'selected' : '' }}>Blade</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="content" name="content" rows="15" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm font-mono text-sm" required>{{ old('content', $certificateTemplate->content) }}</textarea>
                    ...
                    <x-template-preview-modal :route="route('dashboard.templates.certificates.preview')" type="certificate" />
                </div>
            </div>
        </div>
    </div>
